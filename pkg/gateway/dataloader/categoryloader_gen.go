...
type CategoryLoader struct {
	...
	cache map[string]*category.Category
...
func (l *CategoryLoader) Load(key string) (*category.Category, error) {
...
func (l *CategoryLoader) LoadThunk(key string) func() (*category.Category, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*category.Category, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *CategoryLoader) LoadAll(keys []string) ([]*category.Category, []error) {
	results := make([]func() (*category.Category, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	categorys := make([]*category.Category, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *CategoryLoader) LoadAllThunk(keys []string) func() ([]*category.Category, []error) {
	results := make([]func() (*category.Category, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *CategoryLoader) Prime(key string, value *category.Category) bool {
	l.mu.Lock()
	var found bool
...
func (l *CategoryLoader) unsafeSet(key string, value *category.Category) {
	if l.cache == nil {
		l.cache = map[string]*category.Category{}
	}
	l.cache[key] = value
}
